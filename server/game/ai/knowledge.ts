...
- Партия 3–15 человек. Ночь (тайные ходы) → утро (сводка ведущего) → сразу голосование 90 секунд.
...
- Победа города: в живых нет мафии и нет адвоката. Если остались маньяк и мирные — побеждает город.
- Победа мафии: в живых не осталось мирных, ИЛИ чёрных (мафия+адвокат) строго больше, чем города — например 2 мафа и 1 мирный. Адвокат — чёрная команда. Если остались мафия/адвокат и маньяк (города нет) — побеждает мафия. Один главарь и горец — ничья, не победа мафии.
- Ничья: в живых остались только Катани и один из чёрных (мафия или адвокат), либо только главарь и горец.
- Маньяк играет сам за себя и побеждает, только если остался один.
...
- Катани ночью проверяет (личный результат: роль и вердикт «мафия/зло» или «не мафия») ИЛИ стреляет. Адвокат может спрятать мафию от проверки.
- Бомж ночью проверяет и узнаёт точную роль.
- Доктор лечит одного (себя — не чаще раза в три ночи). Спасает от выстрела мафии, маньяка и Катани. Если самурай принял удар на себя — спасает только лечение самурая, не его цели.
- Путана соблазняет любого, кто ходит ночью: ход цели этой же ночью не срабатывает. В чат цель пишет. Клоун и самурай тоже соблазняются.
- Маньяк убивает одного.
- Клоун один раз за игру меняет роли двух игроков.
- Жена комиссара: один выстрел мести, если Катани уже погиб. Этот выстрел доктор не лечит, но если удар принял самурай — доктор может спасти самурая.
- Самурай (город) ночью закрывает другого игрока (не себя). Смертельный удар по цели (мафия, маньяк, Катани, месть жены) переходит на самурая. Выстрел мафии по горцу самурай не перехватывает — мафия горца и так не убивает.
...
- Не повторяй одни и те же фразы и не копируй чужие реплики из чата. Запрещены шаблоны вроде «стоит присмотреться». Говори коротко, по делу, своим словами, как за столом.`;
...
export function buildRoleStrategy(bot: GamePlayer, room: GameRoom): string {
  ...
  if (role === 'doctor') {
    ...
    return [
      'СТРАТЕГИЯ ДОКТОРА:',
      'Лечи вероятного Катани, того, кого могут снять мафия или маньяк, или себя. Лечение спасает и от ошибки Катани. Если думаешь, что самурай закрыл кого-то — лечи самурая: лечение цели его не спасёт.',
      self,
    ].join('\n');
  }

  if (role === 'prostitute') {
    return 'СТРАТЕГИЯ ПУТАНЫ: соблазняй того, кто ходит ночью — дона, Катани, доктора, маньяка, адвоката, бомжа, клоуна, самурая. Ход цели этой ночью не сработает.';
  }
  ...
  if (role === 'samurai') {
    return [
      'СТРАТЕГИЯ САМУРАЯ:',
      'Ночью закрывай того, кого скорее снимут: вероятного Катани, доктора, сильного мирного. Себя закрывать нельзя.',
      'Удар по цели перейдёт на тебя. Доктор спасёт, только если вылечит тебя, не цель. Путана может сбить твой ход.',
    ].join('\n');
  }
...
export function heuristicNightAction(bot: GamePlayer, room: GameRoom): NightAction | null {
  ...
  switch (bot.role) {
    ...
    case 'samurai': {
      const target = pick(targets.filter((p) => p.id !== bot.id));
      return target ? { type: 'guard', targetId: target.id } : null;
    }
    case 'clown': {
...
export function nightInstruction(bot: GamePlayer, room: GameRoom): string {
  ...
  switch (bot.role) {
    ...
    case 'samurai':
      return `Закрой другого игрока (не себя). Смертельный удар по цели примешь на себя.${suffix} JSON: {"action":"guard","targetId":число,"reason":"..."}`;
    default:
      return '';
  }
}
