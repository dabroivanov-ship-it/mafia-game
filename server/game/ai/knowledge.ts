...
- Доктор лечит одного (себя — не чаще раза в три ночи). Спасает от выстрела мафии, маньяка и Катани.
- Путана соблазняет любого, кто ходит ночью: ход цели этой же ночью не срабатывает. В чат цель пишет. Клоун тоже соблазняется.
...
export function buildRoleStrategy(bot: GamePlayer, room: GameRoom): string {
  ...
  if (role === 'doctor') {
    ...
    return [
      'СТРАТЕГИЯ ДОКТОРА:',
      'Лечи вероятного Катани, того, кого могут снять мафия или маньяк, или себя. Лечение спасает и от ошибки Катани.',
      self,
    ].join('\n');
  }

  if (role === 'prostitute') {
    return 'СТРАТЕГИЯ ПУТАНЫ: соблазняй того, кто ходит ночью — дона, Катани, доктора, маньяка, адвоката, бомжа, клоуна. Ход цели этой ночью не сработает.';
  }
