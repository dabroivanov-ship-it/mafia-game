...
export function getMorningIntroMessage(killed: GamePlayer[]): string {
  ...
  const parts = killed.map((p) => {
    ...
    if (p.role === 'commissar') {
      return getPhraseText('morning.killed_commissar', { nick });
    }
    return getPhraseText('morning.killed_player', { nick, role });
  });

  return `${getPhraseText('morning.intro_prefix')} ${parts.join(' ')}`;
}
