...
import { pushReputationNotification } from '../notifications/push.js';
...
router.post('/:userId', authMiddleware, (req, res) => {
  ...
  try {
    ...
    const voterName = voter?.display_name || voter?.username || 'Игрок';

    pushReputationNotification(targetId, value, reputation, voterName);
